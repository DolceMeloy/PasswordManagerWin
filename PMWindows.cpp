// PMWindows.cpp : Определяет точку входа для приложения.
//

#include "framework.h"
#include "PMWindows.h" // Убедитесь, что этот файл существует и корректен
#include "PasswordManager.h" // Подключаем ваш менеджер паролей
#include <sstream>
...
PasswordManager* manager = nullptr;             // Указатель на PasswordManager
    ...
    _In_ int nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_PMWINDOWS, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_PMWINDOWS));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    WNDCLASSEXW wcex = { 0 }; // Инициализируем все поля нулями
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_PMWINDOWS));
    wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
    wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_PMWINDOWS);
...
{
    hInst = hInstance; // Сохраняем экземпляр
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DESTROY:
        delete manager; // Освобождаем память
        PostQuitMessage(0);
        break;
